...
class RoundListWidgetItemDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        if option.state & (QStyle.StateFlag.State_Selected | QStyle.StateFlag.State_MouseOver):
            ...
            painter.fillRect(rect.adjusted(1, 1, -1, -1), QColor("#292929") if isDark else QColor("#DFDFDF"))
        else:
            pen.setColor(QColor("#3A3A3A") if isDark else QColor("#D9D9D9"))
        painter.setPen(pen)
        painter.drawRoundedRect(rect, 8, 8)

        alignment = index.data(Qt.TextAlignmentRole) or Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter
...
class RoundListWidget(QListWidget):
    def __init__(self, parent=None):
        ...
        self.itemDelegate = RoundListWidgetItemDelegate(self)
